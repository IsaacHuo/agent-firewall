...
export async function handleWhatsAppStart(host: AgentShieldApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: AgentShieldApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: AgentShieldApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: AgentShieldApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: AgentShieldApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: AgentShieldApp): string {
...
function resolveGatewayHttpAuthHeader(host: AgentShieldApp): string | null {
...
function buildGatewayHttpHeaders(host: AgentShieldApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: AgentShieldApp) {
...
export function handleNostrProfileToggleAdvanced(host: AgentShieldApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: AgentShieldApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: AgentShieldApp) {
  const state = host.nostrProfileFormState;
